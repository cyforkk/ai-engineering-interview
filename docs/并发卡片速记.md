...
**锁（30 秒）：**  
synchronized 进 Monitor，锁升级偏向→轻量→重量；ReentrantLock 基于 AQS，可中断/公平/多 Condition。  
**易错：** 升级顺序反；Lock 忘记 unlock。

**volatile：** 可见+有序，不保证 i++ 原子；DCL 单例防 new 指令重排。

**线程池（30 秒）：**  
core 满 → 入队 → max 满 → 拒绝；禁用 Executors 无界队列。  
**易错：** 顺序说成先 max 再队列。

**链：** [完整卷](./并发高频面试题与知识点.md) · [面渣](./并发面渣级口述.md) · [结构图·池/锁](./核心结构图.md) · [加分·虚拟线程](./加分-Java-2024-2026.md)
