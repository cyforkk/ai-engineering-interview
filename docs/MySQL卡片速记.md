...
**索引（30 秒）：**  
InnoDB B+；联合索引最左前缀；二级索引叶子是主键 → 常回表；覆盖索引免回表。  
**事务/MVCC：** 版本链 + ReadView；快照读 vs 当前读（加锁读）。  
**易错：** 最左前缀举例错误；回表与覆盖说混；隔离级别只背名不说现象。

**链：** [完整卷](./MySQL高频面试题与知识点.md) · [面渣](./MySQL面渣级口述.md) · [结构图·B+](./核心结构图.md)
